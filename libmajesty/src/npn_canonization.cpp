...
void npn_canonization_sifting_loop( tt& npn, int n, boost::dynamic_bitset<>& phase, std::vector<unsigned>& perm )
{
...
tt npn_canonization_flip_swap( const tt& t, boost::dynamic_bitset<>& phase, std::vector<unsigned>& perm)
{
  /* initialize */
  auto n = tt_num_vars( t );
  phase.resize( n + 1u );
...
tt npn_canonization_sifting( const tt& t, boost::dynamic_bitset<>& phase, std::vector<unsigned>& perm)
{
  /* initialize */
  auto n = tt_num_vars( t );
  phase.resize( n + 1u );
  ...
  auto ns = static_cast<int>(n);
  npn_canonization_sifting_loop( npn, ns, phase, perm );
