...
namespace majesty {
	...
	bestmap eval_matches_area(const logic_ntk& m, const cutmap& cut_map) {
		cout << "Evaluating matches area..." << endl;
		bestmap best(m.nnodes());
		nintmap area_flows(m.nnodes());
		auto nodes = m.nodes();
	...
	cover build_cover(const logic_ntk& m, bestmap& best) {
		cout << "Building cover..." << endl;
		cover nref(m.nnodes());
	...
	unsigned int recursive_deselect(const nodeid nid,
			const vector<ln_node>& nodes, bestmap& best, nintmap& nref) {
		const auto& n = nodes[nid];
		if (n.pi) {
			return 0;
		}
		auto area = 1;
		auto cutnodes = best[nid]->nodes();
		for (auto nodeid : cutnodes) {
			nref[nodeid] -= 1;
			if (nref[nodeid] == 0) {
				area += recursive_deselect(nodeid, nodes, best, nref);
			}
		}
		return area;
	}

	unsigned int recursive_select(const nodeid nid, 
			const vector<ln_node>& nodes, bestmap& best, nintmap& nref) {
		const auto& n = nodes[nid];
		if (n.pi) {
			return 0;
		...
		for (auto nodeid : cutnodes) {
			nref[nodeid] += 1;
			if (nref[nodeid] == 1) {
				area += recursive_select(nodeid, nodes, best, nref);
			}
		}
		return area;
	}
	...
	void improve_cover_exact_area(const logic_ntk& m, const cutmap& cm,
			bestmap& best, nintmap& nref) {
		cout << "Running exact area improvements..." << endl;
		auto nodes = m.nodes();
		for (auto i = 0u; i < nodes.size(); i++) {
	...
	unsigned int cover_size(const logic_ntk& m, const cover& cover) {
		auto size = 0u;
		auto nodes = m.nodes();
		auto nnodes = m.nnodes();
		for (auto i = 0u; i < nnodes; i++) {
	...
	void it_exact_cover(const logic_ntk& m, cover& cover,
			const cutmap& cm, bestmap& best) {
		auto csize = cover_size(m, cover);
		while (true) {
			improve_cover_exact_area(m, cm, best, cover);
